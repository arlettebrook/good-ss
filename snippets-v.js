    ...
    readable.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            const {
                ...
                version,
                isUDP
            } = parseWsPacketHeader(chunk, yourUUID);
...
async function forwardTCP(addrType, host, portNum, rawData, ws, respHeader, remoteConnWrapper, customProxyIP) {
    ...
    if (customProxyIP) {
        ...
        if (proxyConfig && (proxyConfig.type === 'socks5' || proxyConfig.type === 'http' || proxyConfig.type === 'https')) {
            ...
            proxyConfig = parseProxyAddress(proxyIP) || {
                type: 'direct',
                host: proxyIP,
                port: 443
            };
...
function parseWsPacketHeader(chunk, token) {
    ...
    if (formatIdentifier(new Uint8Array(chunk.slice(1, 17))) !== token) return {
        hasError: true,
    ...
    let addrIdx = portIdx + 2, addrLen = 0, addrValIdx = addrIdx + 1,
        hostname = '';
